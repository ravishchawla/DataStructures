...
public class RegexPatterns {
	...
	public static boolean testName(String name){
		return name.matches("([A-Z]{1,2}[a-z]+( |-)*){2,3}");
	}
	...
	public static boolean testPhoneNumber(String phoneNumber){
		return phoneNumber.matches("\\((678|404|770)\\)(\\d){3}-(\\d){4}");
	}
	...
	public static boolean testEmail(String email){
		return email.matches("[a-z]+(\\w|_|\\-|\\.)*@[a-z]+(\\w|\\_|\\-|\\.)*\\.(com|org|net)");
	}
	...
	public static boolean testAddress(String address){
		return address.matches("(\\d){1,5} (\\w|\\.|\\-| )+\n[A-Z]+(\\w|\\.|\\-| )+,* [A-Z]{2} (\\d){5}");
	}
	...
	public static boolean testJavaExecutable(String java){
		return java.matches("public\\ static\\ void\\ main\\(String\\[\\]\\ args\\)");
	}
